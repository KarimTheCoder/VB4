...
/**
 * GRE vocabulary database helper with WAL, indexes, and synchronized transactional writes.
 */
public class GREWordDatabase extends SQLiteOpenHelper {
    private final Object lock = new Object();
    private static volatile GREWordDatabase INSTANCE;
    ...
    public static GREWordDatabase getInstance(Context context) {
        if (INSTANCE == null) {
            synchronized (GREWordDatabase.class) {
                if (INSTANCE == null) {
                    INSTANCE = new GREWordDatabase(context.getApplicationContext());
                }
            }
        }
        return INSTANCE;
    ...
    public void insertData(String word, String fav, String learned, String blacklist, String skip){
        ...
        Log.d("DB_GRE","insertData rowId="+rowId);
    }
    ...
    public void updateFav(String id, String fav){
        ...
        synchronized (lock) {
            ...
            try {
                db.update(TABLE_NAME, cv, "ID = ?", new String[] {id});
                db.setTransactionSuccessful();
                Log.d("DB_GRE","updateFav id="+id+" fav="+fav);
            } finally {
                db.endTransaction();
            }
        }
    ...
    public void updateLearned(String id, String learned){
        ...
        synchronized (lock) {
            ...
            try {
                db.update(TABLE_NAME, cv, "ID = ?", new String[] {id});
                db.setTransactionSuccessful();
                Log.d("DB_GRE","updateLearned id="+id+" learned="+learned);
            } finally {
                db.endTransaction();
            }
        }
